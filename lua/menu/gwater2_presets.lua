...
function gwater2.options.detect_preset_type(preset)
	...
	if util.Decompress(util.Base64Decode(preset)) ~= "" and util.Decompress(util.Base64Decode(preset)) ~= nil then
		if pcall(function()
			util.JSONToTable(util.Decompress(util.Base64Decode(preset)))
		end) then
			return "B64-PI"
		else
			return nil
...
local function get_parameter(param)
	...
	if param_panel.mixer then
		local c = param_panel.mixer:GetColor()
		return { c.r, c.g, c.b, c.a }
	end -- GetColor returns color without metatable, damnit
	if param_panel.check then
		return param_panel.check:GetChecked()
	end
	if param_panel.slider then
		return param_panel.slider:GetValue()
	end
end
local function set_parameter(param, value)
	...
	local param_panel = list_[param:sub(6)]
	if param_panel.mixer then
		return param_panel.mixer:SetColor(Color(value[1], value[2], value[3], value[4]))
	end
	if param_panel.check then
		return param_panel.check:SetChecked(value)
	end
	if param_panel.slider then
		return param_panel.slider:SetValue(value)
	end
end
...
button_functions = {
	...
	apply_preset = function(self)
		...
		for name, _ in pairs(_parameters) do
			paramlist[#paramlist + 1] = "PHYS/" .. name
		end
		for name, _ in pairs(_visuals) do
			paramlist[#paramlist + 1] = "VISL/" .. name
		end
		for name, _ in pairs(_interactions) do
			paramlist[#paramlist + 1] = "INTC/" .. name
		end
			...
			for _, section in pairs(params) do
				for name, control in pairs(section) do
					...
					if control.slider then
						control.slider:SetValue(default)
					end
					if control.check then
						control.check:SetChecked(default)
					end
					if control.mixer then
						control.mixer:SetColor(Color(default:Unpack()))
					end
				end
			end
		end
	...
	create_preset = function(local_presets, name, preset, write, fullid)
		...
		for k, v in SortedPairs(presets) do
			m = tonumber(k:sub(1, 3))
		end
		local selector = local_presets:Add("DButton")
		selector:SetText(
			name .. " (" .. (preset["CUST/Author"] or _util.get_localised("Presets.author_unknown")) .. ")"
		)
		selector:Dock(TOP)
	...
	save_extended = function(self)
		...
		function select_visl:DoClick()
			for _, check in pairs(_checks) do
				if check:GetText():sub(0, 4) ~= self.section then
					continue
				end
				check:SetValue(true)
			end
		...
		function confirm:DoClick()
			...
			if preset["CUST/Master Reset"] then
				for k, v in pairs(preset) do
					...
					if get_parameter(k) ~= gwater2.defaults[param] then
						continue
					end
					preset[k] = nil
				end
...
local function presets_tab(tabs, params)
	...
	local _ = tab:Add("DLabel")
	_:SetText(" ")
	_:SetFont("GWater2Title")
	_:Dock(TOP)
	_:SizeToContents()
	...
	if not file.Exists("gwater2/presets.txt", "DATA") then
		file.Write("gwater2/presets.txt", util.TableToJSON(default_presets))
	end
	local succ, presets = pcall(function()
		return util.JSONToTable(file.Read("gwater2/presets.txt"))
	end)
	if not succ then
