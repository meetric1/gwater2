...
local function init_tab_params(tabs, tab, tab_name, sname, sec, added_params, recursive)
	...
	for name, param in SortedPairs(sec) do
		if param.type == "scratch" then
			added_params[name:sub(5)] =
				util.make_parameter_scratch(pan, tab_name .. "." .. name:sub(5), name:sub(5), param)
		elseif param.type == "color" then
			added_params[name:sub(5)] =
				util.make_parameter_color(pan, tab_name .. "." .. name:sub(5), name:sub(5), param)
		elseif param.type == "check" then
			added_params[name:sub(5)] =
				util.make_parameter_check(pan, tab_name .. "." .. name:sub(5), name:sub(5), param)
		else
...
local function init_tab(tabs, tab_name, image, recursive)
	...
	function _:Paint(w, h)
		...
		draw.DrawText(
			util.get_localised(tab_name .. ".titletext"),
			"GWater2Title",
			5,
			5,
			Color(187, 245, 255),
